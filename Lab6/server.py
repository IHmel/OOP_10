...
class Server():
    ...
    def __init__(self):
        self.socket_server = socket.socket()            # создаем объект сокета сервера
        hostname = socket.gethostname()     # получаем имя хоста локальной машины
        port = 12345                        # устанавливаем порт сервера
        self.socket_server.bind((hostname, port))       # привязываем сокет сервера к хосту и порту
        self.socket_server.listen(5) 
        self.run_server()
    
    def client_check(con):
    ...
    def server_send(self, con, message):
    ...
    def run_server(self):
        ...
        while True:
            client, _ = self.socket_server.accept()     # принимаем клиента
            start_new_thread(self.server_send, (client,)) 
